...
COPY --chown=$MAMBA_USER:$MAMBA_USER env_ansible.yml /opt/conda/env.yml 
RUN --mount=type=cache,target=$MAMBA_ROOT_PREFIX/pkgs,id=mamba_pkgs  micromamba install -y -f /opt/conda/env.yml 
...
COPY packages.txt /opt/conda/packages.txt
RUN --mount=type=cache,target=/var/cache/apt,id=aptdeb12 apt-get update && xargs apt-get install -y < /opt/conda/packages.txt
